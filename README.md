...
A standalone browser page for viewing CLIProxyAPI usage statistics from the management API.
...
The default management endpoint expected by the page is:
...
- Usage queue records are consumed by the management endpoint after they are read, so sync the page before clearing browser data.
